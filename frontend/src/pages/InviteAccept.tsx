...
type State =
  ...
  | { status: 'ready'; invite: Invite }       // logged out — show sign in/up buttons
  | { status: 'accepting'; invite: Invite }   // in flight — invite kept so UI stays rendered
...
export default function InviteAccept() {
  ...
  // Step 1: fetch the invite on mount
  useEffect(() => {
  ...
  // Step 2: auto-accept as soon as we have a valid invite AND a logged-in user
  useEffect(() => {
  ...
  // Only re-run when user logs in (goes from null → value) or the invite becomes ready
  ...
  if (state.status === 'loading' || state.status === 'accepting') {
    return (
      <Screen>
        <p className="text-gray-400">{state.status === 'loading' ? 'Loading invite...' : 'Joining group...'}</p>
      </Screen>
    )
  }
    ...
    return (
      <Screen>
        <div className="w-full max-w-sm bg-white rounded-2xl shadow-sm border border-gray-200 p-8 text-center">
          ...
          <Link to="/dashboard" className="text-indigo-600 hover:underline text-sm">Go to dashboard</Link>
        </div>
      </Screen>
    )
  }
  ...
  // status === 'ready' and user is NOT logged in — show auth options
  const { invite } = state
  return (
    <Screen>
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-sm border border-gray-200 p-8 text-center">
        ...
        <div className="space-y-3">
          <Link
            ...
            className="block w-full bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg py-2 text-sm transition-colors"
          >
            Create account
          </Link>
          <Link
